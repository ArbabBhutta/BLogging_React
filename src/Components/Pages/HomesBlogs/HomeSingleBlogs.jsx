...
export default function HomeSingleBlogs() {
  ...
  useEffect(() => {
    ...
    let {B_id}=useParams()
    let homie_blog=BlogInHome.find((p)=> p.B_id===Number(B_id))
...
const [allcmnt,setAllcmnt]=useState([])
  ...
  return (
    <div>
    <div >
      <img className='singleBlogsImg mt-3 mb-3' src={  homie_blog.B_img} alt="Blog_photo"  width='100%' height='700px'/>
      <h1 style={{textAlign:'center'}}>{  homie_blog.B_Head}</h1>
      <p style={{padding:'30px'}}>{  homie_blog.B_disc}
      <br />
