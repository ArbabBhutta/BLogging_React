...
export default function SingleTec() {
  ...
  useEffect(() => {
    ...
    let {B_id}=useParams()
    let Tech_blog_detail=TechBlog.find((p)=> p.B_id===Number(B_id))
...
const [allcmnt,setAllcmnt]=useState([])
  ...
  return (
    <div>
      <div >
        <img className='singleBlogsImg' src={Tech_blog_detail.B_img} alt="Blog_photo"  width='100%' height='700px'/>
        <h1 style={{textAlign:'center'}}>{Tech_blog_detail.B_Head}</h1>
        <p style={{padding:'30px'}}>{Tech_blog_detail.B_disc}
        <br />
        <button className='mt-3 mb-3 btn btn_ReadMore'  onClick={addcomnt}>Leave a comment</button> <br/>
   
        {newcomnt && signed ? <div>
          <form onSubmit={submitHandler}>
            <input type="text" onChange={(e)=>setcmntinput(e.target.value)} value={cmntinput} />
            <br />
