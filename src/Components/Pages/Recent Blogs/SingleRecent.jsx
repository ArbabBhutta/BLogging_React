...
export default function SingleRecent() {
  ...
  useEffect(() => {
    ...
    let {B_id}=useParams()
    let recenttBlog=RecentBlogs.find((p)=>p.B_id===Number(B_id))
...
const [allcmnt,setAllcmnt]=useState([])
  ...
  return (
    <div>
    <div >
      <img className='singleBlogsImg' src={recenttBlog.B_img} alt="Blog_photo"  width='100%' height='700px'/>
      <h1 style={{textAlign:'center'}}>{recenttBlog.B_Head}</h1>
      <p style={{padding:'30px'}}>{recenttBlog.B_disc}
      <br />
